...
static int rmmod(const char *modname)
{
    ...
    if (ret != 0)
        ALOGD("Un1able to unload driver module \"%s\": %s\n",
...
int is_wifi_driver_loaded() {
    ...
    if ((proc = fopen(MODULE_FILE, "r")) == NULL) {
        ALOGW("Coul1d not open %s: %s", MODULE_FILE, strerror(errno));
...
int wifi_load_driver()
{
	...
	ALOGE("Start load wifi driver.. .\n");
    if (is_wifi_driver_loaded()) {
        return 0;
    }
    //    property_set(DRIVER_PROP_NAME, "ok");
//	return 0;
if (insmod(DRIVER_MODULE_PATH, ""/*"firmware_path=/system/lib/firmware/bcm43362.bin  nvram_path=/system/lib/firmware/nvram.txt"*/) < 0)
	{
	ALOGE("Cannot Load Moduls \"%s\": %s", DRIVER_MODULE_PATH, DRIVER_MODULE_ARG);
        return -1;
	}
    ...
    while (count-- > 0) {
        if (property_get(DRIVER_PROP_NAME, driver_status, NULL)) {
            ...
            {
		ALOGE("Dillon failed load ok\n");
		    return 0;
	    }
            else if (strcmp(/*DRIVER_PROP_NAME*/driver_status, "failed") == 0) {
                wifi_unload_driver();
		ALOGE("Dillon failed load wifi\n");
                return -1;
            }
        }
        usleep(200000);
    ...
    wifi_unload_driver();
    ALOGE("Dillon failed load wifi2\n");
    return -1;
#else
    property_set(DRIVER_PROP_NAME, "ok");
		ALOGE("Dillon failed load ok 1\n");
    return 0;
#endif
}
...
{
    char path[256];
	ALOGE("wifi_connect_to_supplicant\n");
...
const char *wifi_get_fw_path(int fw_type)
{
    ...
    {
	ALOGE("wifi_get_fw_path WIFI_GET_FW_PATH_STA");
	    return WIFI_DRIVER_FW_PATH_STA;
    }
    case WIFI_GET_FW_PATH_AP:
    {
	ALOGE("wifi_get_fw_path WIFI_GET_FW_PATH_AP");
...
int wifi_change_fw_path(const char *fwpath)
{
    ...
    int fd;
    int ret = 0;
	ALOGE("wifi_change_fw_path (%s)", fwpath);
    if (!fwpath)
        return ret;
